...
class LFFI(nn.Module):
    ...
    def forward(self, x, txt):
        ...
        vis = self.augment(x)
        vis2 = self.norm1(vis)
        vis2_v, _ = self.cross_attn1(query=self.vis_pos(vis2), key=self.txt_pos(txt), value=txt)
        vis2_l, _ = self.cross_attn2(query=self.txt_pos(txt), key=self.vis_pos(vis2), value=vis2)
...
class Decoder(nn.Module):
    def __init__(self, in_channels, out_channels, spatial_size, text_len, embed_dim=768) -> None:
        ...
        self.psr_layer_norm = nn.LayerNorm(embed_dim)
        self.psr_layer_mlp = nn.Sequential(
    ...
    def forward(self, vis, skip_vis, txt):
        if txt is not None:
            ...
            txt_res = txt
            txt_mlp_out = self.psr_layer_mlp(self.psr_layer_norm(txt))
