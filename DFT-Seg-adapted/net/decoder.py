...
class LFFI(nn.Module):
    ...
    def forward(self, x, txt):
        ...
        tExt:[B,L,C]
        '''
        txt = self.text_project(txt)  # 文本
        vis = self.augment(x)
        vis2 = self.norm1(vis)  # 图像
        vis2_v, _ = self.cross_attn1(query=self.vis_pos(vis2),  # 图像加权的文本
                                     key=self.txt_pos(txt),
                                     value=txt)
        vis2_l, _ = self.cross_attn2(query=self.txt_pos(txt),  # 文本加权的图像
                                     key=self.vis_pos(vis2),
                                     value=vis2)
        vis2_v = self.norm2(vis2_v + vis2)  # 文本+图像
        vis2_l = self.norm3(vis2_l + txt)  # 图像＋文本
...
class Decoder(nn.Module):
    def __init__(self, in_channels, out_channels, spatial_size, text_len, embed_dim=768, input_text_len=24) -> None:
        super().__init__()

        # 💡 [修改] 升级为标准的带残差的 MLP 结构 (先升维，再降维)
        self.text_update_norm = nn.LayerNorm(embed_dim)
    ...
    def forward(self, vis, skip_vis, txt):
        if txt is not None:
            # 💡 [修改] 在当前层，加入残差连接保护文本特征主体，只学习细节增量
            txt_res = txt
            txt_mlp_out = self.text_update_mlp(self.text_update_norm(txt))
            txt = txt_res + txt_mlp_out

            # 将更新后的专属文本特征送入 LFFI 融合
